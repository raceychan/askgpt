...
class DeliveryBroker[TMessages](abc.ABC):
    ...
    @abc.abstractmethod
    def register(self, subscriber: Receivable[TMessages]) -> None:
        raise NotImplementedError
...
class DeliveryQueue[TMessage](DeliveryBroker[TMessage]):
    def __init__(self, maxsize: int = 1):
        ...
        self._subscribers: set[Receivable[TMessage]] = set()

    @property
    def subscribes(self) -> set[Receivable[TMessage]]:
    ...
    def register(self, subscriber: Receivable[TMessage]) -> None:
...
try:
    ...
    #     async def _publish_confirmation(self, res, msg_id: str) -> None:
    #         assert res
